...
void Scanner::Init() {
	...
	for (i = 43; i <= 43; ++i) start.set(i, 14);
	for (i = 45; i <= 45; ++i) start.set(i, 14);
	start.set(39, 6);
	start.set(35, 8);
	start.set(46, 10);
	start.set(61, 16);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 10:
			if ((ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_11;}
			else {goto case_0;}
		case 11:
			case_11:
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_11;}
